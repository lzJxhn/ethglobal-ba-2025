...
async function sendSupply(hre: HardhatRuntimeEnvironment, args: SendArgs) {
    ...
    const stargateAbi = await hre.artifacts.readArtifact('@layerzerolabs/oft-evm/contracts/interfaces/IOFT.sol:IOFT')
    const stargatePool = await ethers.getContractAt(stargateAbi.abi, args.srcOft, signer)
    logger.debug(`Connected to Stargate Contract: ${stargatePool.address}`)
    ...
    const [, , receipt] = await stargatePool.quoteOFT(sendParamTuple)
    ...
    const quoteSendResult = await stargatePool.quoteSend(sendParamTuple, payInLzToken)
    ...
    // 4. Ensure ERC20 approval if stargatePool is token-based
    ...
    if (tokenAddress !== ethers.constants.AddressZero) {
        ...
        const currentAllowance = await erc20.allowance(signer.address, stargatePool.address)
        logger.info(`Checking current allowance.`)

        if (currentAllowance.lt(amountLD)) {
            logger.info(`Approving stargatePool to spend tokens.`)
            const approveTx = await erc20.approve(stargatePool.address, amountLD)
    ...
    const tx = await stargatePool.send(sendParamTuple, messagingFee, refundAddress, {
        value: valueToSend,
    })
...
task('aave:supply', 'Sends tokens through Stargate and composes into AaveV3Composer')
    .addParam('srcOft', 'Stargate pool address')
